...
class NotifierService < ApplicationService
  ...
  def search_places(scope)
    scope.or(Search.where("search_place IS NOT NULL AND trim(search_place) != '' AND :event_place ILIKE '%' || search_place || '%'", event_place: @event.place))
  end
  ...
  def search_subjects(scope)
    scope.or(Search.where("search_subject IS NOT NULL AND trim(search_subject) != '' AND :event_body ILIKE '%' || search_subject || '%'", event_body: @event.body.join))
  end
end
